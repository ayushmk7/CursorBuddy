...
|-------|------------|------|
| **Backend (PRD)** | **OpenClaw** (required) + **optional bridge** | Session orchestration, tools (`vscode_probe_state`, `waveclick_emit_envelope`, …), model/realtime routing **inside** OpenClaw, policy, audit (bridge), minting upstream params ([`docs/openapi.yaml`](../../docs/openapi.yaml)). |
